...
if api_key:
    ...
    if not df.empty:
        ...
        signal_time = last_candle.name.strftime('%Y-%m-%d %H:%M')

        if last_candle['Buy']:
            st.success(f"✅ BUY signal at {signal_time} IST (latest candle)")
        elif last_candle['Sell']:
            st.error(f"❌ SELL signal at {signal_time} IST (latest candle)")
        else:
            st.info(f"No BUY/SELL signal on latest candle at {signal_time} IST")
